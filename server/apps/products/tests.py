...
from .selectors import get_active_products, get_active_categories, get_product_by_slug
...
def make_admin():
    return User.objects.create_superuser(
        email='admin@test.com', phone='01000000000', password='adminpass'
    )
...
class ProductSelectorsTest(TestCase):
    def setUp(self):
        ...
        self.inactive = make_product(self.category, name='Inactive Product', is_active=False)
...
class ProductAPITest(TestCase):
    ...
    def test_product_detail_returns_200(self):
        response = self.client.get(f'/api/products/products/{self.phone.slug}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_product_detail_inactive_returns_404(self):
        response = self.client.get(f'/api/products/products/{self.inactive.slug}/')
    ...
    def test_filter_by_category(self):
        response = self.client.get(
            f'/api/products/products/?category__slug={self.cat_clothing.slug}'
        )
        names = [p['name'] for p in response.data['results']]
    ...
    def test_filter_by_min_price(self):
        response = self.client.get('/api/products/products/?min_price=900')
        names = [p['name'] for p in response.data['results']]
        self.assertIn('Laptop', names)
        self.assertNotIn('Smartphone', names)

    def test_filter_by_max_price(self):
        response = self.client.get('/api/products/products/?max_price=100')
        names = [p['name'] for p in response.data['results']]
        self.assertIn('T-Shirt', names)
    ...
    def test_combined_filters(self):
        response = self.client.get(
            f'/api/products/products/?search=Laptop&category__slug={self.cat_electronics.slug}&min_price=500'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        names = [p['name'] for p in response.data['results']]
        self.assertIn('Laptop', names)
...
class AdminProductAPITest(TestCase):
    ...
    def test_admin_create_product(self):
        ...
        response = self.client.post('/api/products/admin/products/', data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['name'], 'New Laptop')

    def test_admin_update_product(self):
        product = make_product(self.category)
        data = {'name': 'Updated Name', 'description': product.description,
                'price': str(product.price), 'stock': product.stock,
                'category_id': self.category.pk}
        response = self.client.put(
            f'/api/products/admin/products/{product.slug}/', data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_admin_deactivate_product(self):
        ...
        response = self.client.post(
            f'/api/products/admin/products/{product.slug}/deactivate/'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        product.refresh_from_db()
        self.assertFalse(product.is_active)
    ...
    def test_admin_update_stock(self):
        product = make_product(self.category, stock=5)
        response = self.client.post(
            f'/api/products/admin/products/{product.slug}/update_stock/',
            {'quantity': 50}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        product.refresh_from_db()
    ...
    def test_non_admin_cannot_access_admin_endpoints(self):
        user = User.objects.create_user(
            email='user@test.com', phone='01111111111', password='pass'
        )
        self.client.force_authenticate(user=user)
        response = self.client.get('/api/products/admin/products/')
    ...
    def test_admin_deactivate_category(self):
        ...
        response = self.client.post(
            f'/api/products/admin/categories/{cat.slug}/deactivate/'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        cat.refresh_from_db()
        self.assertFalse(cat.is_active)
